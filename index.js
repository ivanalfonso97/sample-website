...
colorSchemeArr.map(colorScheme => {
    colorSchemeHtml += `
    <option class="option" value="${colorScheme}">${colorScheme}</option>`
})
...
function generateColors(color, scheme) {
    ...
    fetch(`https://www.thecolorapi.com/scheme?hex=${color}&mode=${scheme}`)
        .then(res => res.json())
        .then(data => {
            data.colors.map(color => colorArr.push(color.hex.value))
            renderColors()
        })
}
...
function renderColors() {
    ...
    colorArr.map(color => {
        colorDisplayHtml += `
            <div id="${color}" class="color" onClick="copyText(id)">
